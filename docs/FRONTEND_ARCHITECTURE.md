...
| ----------------- | --------------------------------- | --------------------------------------------------------------------------- |
| `AppLayout.vue`   | Shared sidebar at ≥992px          | `AppShell`: compact header, role-derived top navigation, and five-item dock |
| `AdminLayout.vue` | Shared `.admin-sidebar` at ≥992px | `AppShell`: compact header, admin top navigation, and five-item dock        |
| `AppShell.vue`    | Sidebar at ≥992px                 | Owns authenticated shell, account actions, safe content offset, and dock    |
...
1. **Shared responsive shell** — `AppShell` is the single authenticated shell. It renders a desktop sidebar at ≥992px and a compact header, role-derived horizontal navigation, and five-action dock below that breakpoint.
2. **No business-navigation drawer** — Secretary routes stay within `AppLayout`; mobile business modules are always available from `RoleTopNavigation`, not a hamburger or offcanvas.
...
| -------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| HIGH     | AdminAccess, AuditTrail, AdminChatQueries, TenantOperations, and MyDisciplinary have partially hardcoded English | Multiple views                                 |
| MEDIUM   | Remaining operational tables beyond contributions and finance need progressive `ResponsiveDataView` adoption     | Admin table views                              |
