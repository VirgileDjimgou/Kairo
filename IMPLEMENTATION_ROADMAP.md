...
- Operations journal clarity: the protected journal shared with every elected office role resolves each author to their display name or email and tenant role(s), and translates common actions and outcomes into localized, human-readable summaries rather than exposing internal identifiers.
...
- Member lifecycle refinement: every elected office role can register, edit, pause, or reactivate a membership from the member list; deletion remains reserved for the president and secretary general. Membership outcomes are recorded in the shared operations journal, with mobile-safe stacked action buttons.
- Member registration refinement: the president/secretary form now validates a complete structured German address, preselects Germany, and previews the automatically generated unique member email. The API stores the address tenant-scoped and generates the final unique `@combis.org` email server-side from the login identifier or display name.
- Member and finance visibility refinement: a selected directory member opens a responsive detail panel with role-authorized finance and disciplinary histories. Finance workspaces now use progressive member search and selected-member statements; receipt declarations awaiting validation are presented in the treasurer finance workspace rather than the declaration form.
