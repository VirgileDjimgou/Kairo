...
- Post-Sprint 99 stabilization: the client now provides centralized, dismissible top-right operation feedback for all API mutations and failures; sanitized failures are retained in a tenant-scoped operation journal visible to every elected office role. Visual proof is stored under `apps/web/artifacts/role-workflow-proof/2026-07-29/`.
...
- Operations journal clarity: every elected office role now sees the author’s display name or email plus tenant role(s), and localized human-readable action and outcome summaries instead of internal actor, tenant, and request identifiers.
...
- Membership lifecycle: every elected office role can register, edit, pause, or reactivate a member; only the president and secretary general can delete a profile. Each membership mutation is recorded in the shared operations journal with a human-readable outcome. Phone actions are stacked as full-width controls for safe touch use.
- Member registration refinement: the new-member form now requires a structured German address (street, house number, postal code, city, country), keeps Germany preselected, and displays a server-generated unique `@combis.org` address derived from the login identifier or display name. Phone and login identifier remain optional.
- Member and finance visibility refinement: selecting a member now opens a responsive read-only profile panel with identity, permitted finance history, and permitted disciplinary history. Treasurer and auditor finance workspaces provide progressive member search; the treasurer validation queue is now located at the top of the finance workspace.
- Financial income refinement: office declarants can classify a receipt as a member contribution, donation, patronage/sponsorship, tournament proceeds, or another income. A non-member income requires a named source and remains pending until the treasurer validates it, without changing a member contribution balance.
- Treasury custody refinement: validating a receipt updates the member or disciplinary record immediately but opens a separate treasury-custody trail. The declaring office holder reports the handover, and the treasurer alone confirms receipt in the association cashbox. The default handover reminder deadline is two days and can be set from one to seven days at validation.
...
- Notification delivery extension: authenticated, tenant-isolated inboxes now use a transactional outbox, support multiple authorised profiles on one browser installation, and offer opt-in Web Push with VAPID. Push content is intentionally generic; finance and disciplinary details remain behind authentication. Receipt declarations, treasurer decisions, disciplinary updates, public announcements, and public events create inbox events. A president/principal administration endpoint lists recipients who still have unread notifications but no active Push subscription. Telegram and WhatsApp are intentionally outside this delivery flow.
- Notification and navigation stabilization: the client now rejects every obsolete HTTP API base URL on an HTTPS page, uses the same-origin API proxy, registers the service worker explicitly before Push subscription, and does not query tenant disciplinary records from the receipt form unless the current role is authorised and explicitly selects a disciplinary payment. A transient Push-provider failure can no longer prevent the authenticated inbox event from completing.
- Treasury expense and budget refinement: only the treasurer can record a tenant-scoped cash outflow. Each entry is audit-logged and immediately reduces the realized annual available budget. The treasurer workspace now presents responsive income and expense donut charts, separating member contributions, donations, sponsorship, tournament income, disciplinary payments, and other income from sport/equipment, fuel/transport, tournament, cultural, administration, and other outflows.
- Recovery and backup hardening: the core deployment now creates encrypted, SHA-256 verified and signed PostgreSQL/MinIO recovery archives, records tenant-scoped backup runs, schedules daily backups, archives PostgreSQL WAL, blocks critical imports/deletions without a verified safety snapshot, and provides a role-protected recovery centre plus Windows/Docker recovery scripts. The external S3-compatible destination remains an operator configuration step; no cloud storage credentials are embedded in the product.
- Assisted access recovery: the president, vice president, secretary general and principal administrator can issue a one-time 48-hour password to an active member. It is shown once, requires immediate replacement, revokes existing sessions, invalidates pending reset tokens, and is recorded in the tenant audit trail. Members can also change their own password from Account Security without email delivery.
