<template>
  <div class="d-flex min-vh-100 shell-bg">
    <aside class="sidebar d-none d-md-flex flex-column p-3">
      <div class="brand d-flex align-items-center gap-2 mb-4 px-1">
        ...
        <span class="fw-bold text-dark">Kairo</span>
      </div>
      ...
      <header class="topbar sticky-top bg-white border-bottom">
        ...
        >
          <div>
            ...
            <div class="fw-semibold">{{ tenantStore.currentTenantName }}</div>
          </div>
          ...
          <div class="d-flex align-items-center gap-2 ms-auto">
            <div class="dropdown tenant-switcher">
              ...
              <ul class="dropdown-menu dropdown-menu-end shadow-sm">
                <li v-for="tenant in tenantStore.tenants" :key="tenant.id">
                  <button
                    class="dropdown-item"
                    @click="tenantStore.selectTenant(tenant.id)"
                  >
                    <div class="fw-medium">{{ tenant.name }}</div>
                    <div class="small text-muted">{{ tenant.slug }}</div>
                  </button>
                </li>
              </ul>
            </div>
