...
function makePrincipalAdminResponse(modules: ModuleToggles) {
  return {
    id: 'user-principal-1',
...
async function mockPrincipalAdminOverview(page: Page, modules: ModuleToggles) {
  await page.addInitScript(() => {
  ...
  await page.route('http://localhost:8000/api/v1/auth/me', async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify(makePrincipalAdminResponse(modules)),
    })
  })
      ...
      body: JSON.stringify([
        {
          id: 'audit-1',
          tenant_id: 'tenant-demo-1',
          actor_user_id: 'user-principal-1',
          module_key: 'documents',
          action: 'create',
          entity_type: 'document',
          entity_id: 'doc-1',
...
test.describe('Admin overview', () => {
  test('renders a real operations hub for admins', async ({ page }) => {
    ...
    await expect(page.getByTestId('admin-overview-hub-label')).toHaveText('Administration')
    await expect(page.getByTestId('admin-overview-title')).toHaveText('Admin overview')
  ...
  test('renders the principal admin control plane label for principal admins', async ({ page }) => {
    await mockPrincipalAdminOverview(page, {
      membership: true,
      contributions: true,
      policies: true,
      disciplinary: true,
    ...
    await expect(page).toHaveURL(/\/admin$/)
    await expect(page.getByTestId('admin-overview-hub-label')).toHaveText('Principal administration')
    await expect(page.getByTestId('admin-overview-title')).toHaveText('Principal admin overview')
    await expect(page.getByTestId('admin-layout-title')).toHaveText('Principal Admin Control Plane')
  })
})
